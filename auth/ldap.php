<?php
include_once "userPlugin.php";

class errorLDAP extends Exception {};
class errorLDAPConn extends Exception {};
class errorLDAPBind extends Exception {};
...
class userLDAP extends userPlugin
{
     ...
     */
    public function __construct($ldapServer, $baseDN, $user=null, $passwd=null)
    {
           $this->baseDN = $baseDN;
           ...
           if (!empty($user) && !empty($passwd)) {
              if (!($bind=ldap_bind($this->connect, $user, $passwd)))
                 throw new errorLDAPBind("Error al fer el bind al LDAP: $ldapServer");
           } else {
              if (!($bind=ldap_bind($this->connect)))
                 throw new errorLDAPBind("Error al fer el bind al LDAP: $ldapServer");
           }
    }
