...
class clientTimecp5000 extends clientTimeBase
{
     ...
     */
    public function __construct(string $srv_ip, int $srv_port = 1000)
    {
    ...
    /**
     * Realitzem la connexió TCP/IP amb el servidor
     * @throws socketBlock
    ...
    /**
     * Envia una trama XML al servidor, i agafem la resposta
     * @param $xml string Petició XML a enviar
     * @return int Estat
     */
    private function sendXML(string $xml) : int
    {
            if(self::DEBUG)
               echo "[ TX ] $xml\n";
     ...
     * @return string
     */
      private function mountArgs($l_args) : string
      {
               $retval = "";
               if(is_array($l_args))
               {
     ...
     * @throws Exception
     */
      public function sendCommand($type, $cmd, $l_args="") : int
      {
               if(strlen($cmd)==0)
                   return -1;
     ...
     * @throws Exception
     */
      public function sendCommandsBlock() : int
      {
     ...
     * @throws Exception
     */
    private function processResponse() : int
      {
               ...
               while(!$final)
               {
                     ...
                     if (!($domResponse = (new \DOMDocument)->loadXML($this->buff)))
                     ...
                     $this->listResponses[] = $this->campsRes;
