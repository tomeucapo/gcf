...
use DOMNode;
use Exception;
use gcf\terminals\fingerPrintConvert;
...
class clientTimecp6000 extends clientTimeBase
{
    ...
    public function sendCommand(string $type, string $cmd, ?array $l_args = null) : int
    {
        ...
        $this->initRequest();        // Inicialitzem la request, el DOM amb un request buid
        if (is_array($l_args)) {
    ...
    private function extractFields(DOMNamedNodeMap $attributes) : array
    {
        $l_fields = [];
    ...
    private function processResponse()
    {
		...
		if (!$domResponse->loadXML($this->buff))
