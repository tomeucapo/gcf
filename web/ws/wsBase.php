    ...
    public function __construct($errNumber)
    {
           switch ($errNumber)
           {
               case JSON_ERROR_DEPTH:
                    $this->message = 'Excedido tama�o m�ximo de la pila';
                    break;
               case JSON_ERROR_STATE_MISMATCH:
                    $this->message = 'Desbordamiento de buffer o los modos no coinciden';
                    break;
               case JSON_ERROR_CTRL_CHAR:
                    $this->message = 'Encontrado car�cter de control no esperado';
                    break;
               case JSON_ERROR_SYNTAX:
                    $this->message = 'Error de sintaxis, JSON mal formado';
                    break;
               case JSON_ERROR_UTF8:
                    $this->message = 'Caracteres UTF-8 malformados, posiblemente est�n mal codificados';
                    break;
               default:
...
abstract class webServiceBase implements webservice
{
         ...
         final private function getData()
         {
                ...
                switch($this->method) 
                {
                     ...
                     case 'DELETE': parse_str($_SERVER["QUERY_STRING"], $this->dataRequest);
                                    break;
                }             
         }
